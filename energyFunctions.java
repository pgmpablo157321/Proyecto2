...
public class energyFunctions {
	...
	private static int function(int i, int j, Color img[][]) {
		...
		int m = img[0].length;
		int cont = 0;
		int blue = Math.abs(img[i][j].getBlue());
		int red = Math.abs(img[i][j].getRed());
		int green = Math.abs(img[i][j].getGreen());
		if (i > 0) {
			ans += blue - img[i - 1][j].getBlue();
			ans += red - img[i - 1][j].getRed();
		...
		if (j > 0) {
			ans += blue - img[i][j - 1].getBlue();
			ans += red - img[i][j - 1].getRed();
	...
	private static int function2(int i, int j, Color img[][]) {
		...
		if (i > 0 && j > 0) {
			ans += blue - img[i - 1][j - 1].getBlue();
			ans += red - img[i - 1][j - 1].getRed();
